...
nwdsl `0.2` の全フィールドの意味・制約・例。構文の機械可読な定義は `nwdsl schema` で生成する JSON Schema、意味的制約はバリデーション規則(後半)を参照。

- 未定義のフィールドを書くとスキーマ違反になる(タイポ検出のため `extra: forbid`)
- ID はファイル内での参照キー。命名は自由だが、機器は `拠点-役割連番` (例: `hq-rt01`) を推奨
- `0.1` のファイルもそのまま受理される。ただし `0.2` で `devices[].redundancy_group`(文字列)は
  **廃止**され、トップレベルの `redundancy_groups` エンティティに移行した([ADR-0010](adr/0010-redundancy-groups.md))

## トップレベル

| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `nwdsl` | string | ✓ | スキーマバージョン。`"0.2"`(現行)/ `"0.1"` |
...
| `redundancy_groups` | list | - | 冗長グループ (HSRP/VRRPペア・スタック) |
| `domains` | list | - | ルーティングドメイン (OSPFエリア/BGP AS/VRF等) |
| `redistributions` | list | - | ルート再配布 (ドメイン間の関係) |
...
冗長構成 (HSRP/VRRPペア・スタック) は `redundancy_groups` (下記) で宣言する
(`0.1` にあった `redundancy_group` 文字列フィールドは廃止)。
...
| `direction` | enum | - | `both` (default) = 無向 / `forward` = `endpoints[0]→endpoints[1]` の矢印で描く。**logical のみ**。デフォルトルート等の向きの表現 ([ADR-0012](adr/0012-directed-logical.md)) |
...
**論理図でのセグメント内包表示 (D2のみ)**: `role: server` の機器が、そのセグメントだけを参照し(他のセグメント・回線に属さない)場合、セグメントの箱の外側にGW接続として描く代わりに、セグメントの箱の中に入れ子で描画する。複数セグメントに跨る機器(L3スイッチ等)やルーターは今まで通り拠点レベルに置き、箱へはGW接続の線で繋ぐ。内蔵SVGエンジンは今のところ対象外(単一ノードとして描画される)。

## redundancy_groups (冗長グループ)

HSRP/VRRPペアやスタックを構造化して宣言する([ADR-0010](adr/0010-redundancy-groups.md))。

| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `id` | string | ✓ | 冗長グループID (一意) |
| `kind` | enum | - | `fhrp` (default) = HSRP/VRRP等のGW冗長 / `stack` = スタック・シャーシ冗長 |
| `name` | string | - | 表示名。stack を畳んだときのノード名・枠ラベルに使用 (省略時は id / 自動生成) |
| `protocol` | enum | - | `hsrp` / `vrrp` / `glbp` / `other` (**kind: fhrp のみ**)。枠ラベルに表示 |
| `group` | int | - | FHRPグループ番号 (**kind: fhrp のみ**) |
| `vip` | string | - | 仮想IP (**kind: fhrp のみ**。CIDRなしの単一アドレス)。所属セグメントはCIDR包含で自動導出され、IP設計表のGW列に `VIP x.x.x.x` として載る |
| `members` | list | ✓ | メンバー機器 (2台以上)。`{device: <機器ID>, role: active/standby}` (role は任意) |
| `description` | string | - | 補足 |

図での表現:

- **点線枠 (全エンジン)**: 同一拠点内に2台以上のメンバーが描かれるとき、点線枠+ラベル (例: `HSRP grp1 VIP 10.1.0.1`) で囲まれる。メンバーが重複するグループ (同一ペアのVLAN別HSRP等) は1枠に統合される
- **Act/Sbyバッジ**: `role` を持つFHRPメンバーはノード名に `(Act)` / `(Sby)` が付く
- **スタック畳み**: `kind: stack` は論理ビューで1ノード (`name`、ラベル2行目に `スタック×N`) に畳まれる。スタック間リンクは消え、両筐体へのクロススタックLAGは既存の×N束ねに集約される。制御は `views[].merge_stacks`
- FHRPの priority 値・preempt・トラッキングはスコープ外 (role の2値まで)。paths の `protocol: HSRP` 注記との整合は検査しない (障害時の standby 昇格を書くために食い違ってよい)

書き方の例は [patterns.md §1](patterns.md) (HSRPペア)・実演は `examples/stack-core`(スタック)を参照。
...
|---|---|---|---|
| `id` | string | ✓ | ドメインID (一意) |
| `name` | string | △ | 表示名 (例: `OSPF Area 0`)。凡例・面塗りラベルに使用。省略時は `protocol`/`area`/`asn` から自動生成 (**name か protocol のどちらかは必須**) |
| `protocol` | enum | - | `ospf` / `bgp` / `eigrp` / `rip` / `static` / `other` ([ADR-0011](adr/0011-routing-domains.md)) |
| `area` | int / string | - | OSPFエリア識別子 (**protocol: ospf のみ**。`0` / `"0.0.0.0"` 両形式可) |
| `asn` | int | - | BGP AS番号 (**protocol: bgp のみ**) |
| `description` | string | - | 補足 |

表示名の自動生成: `{protocol: ospf, area: 0}` → `OSPF Area 0`、`{protocol: bgp, asn: 65000}` → `BGP AS65000`。`name` を書けばそれが優先される (エリアタイプ等の注記は name で: `OSPF Area 1 (NSSA)`)。
...
機器のドメイン所属は logical link の端点であることから導出される (機器への直接所属フィールドは二重管理になるため設けない。スタブ機器も GW への logical link を書く)。

## redistributions (ルート再配布)

ドメイン間の再配布関係([ADR-0011](adr/0011-routing-domains.md))。

| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `from` | string | ✓ | 再配布元ドメインID |
| `to` | string | ✓ | 再配布先ドメインID (from と別) |
| `devices` | list | ✓ | 再配布を実施する機器ID (1台以上。冗長ASBRは複数書く) |
| `mutual` | bool | - | true = 相互再配布 (`⇄`)。default false = from→to の一方向 (`→`) |
| `description` | string | - | 補足 |

図では `devices` の機器ノードに `再配布: OSPF Area 0 ⇄ BGP AS65000` のバッジ行が付く (from/to いずれかのドメインがそのビューに現れる場合のみ。`collapse_sites` の概要図には出ない)。表は `routing` セクションに一覧が出る。
...
| `merge_stacks` | bool | - | `kind: stack` の冗長グループを1ノードに畳む ([ADR-0010](adr/0010-redundancy-groups.md))。**省略時は純粋なL3ビュー (show_l3 の自動判定と同条件) で自動有効**。物理図で明示 true にするとスタック集約後の配線図が描ける (`examples/stack-core` の `cabling-merged`) |
...
## スコープ (v0.2) — 何を入れて、何を入れないか

nwdsl は「構成図・経路図・設計書表を生成するための最小NWモデル」に意図的に留めている。views は抽出・畳み込み・強調の宣言までとし、座標・色は今後も追加しない (図ツールのYAML再発明を防ぐため)。

| 入れる | 入れない (拒否) |
|---|---|
| 拠点 / 機器 / IF / 回線契約 / 接続 / L3セグメント / 冗長グループ (FHRP/スタック) / ルーティングドメイン / 再配布 / 通信経路 / ビュー | Config全文、ACL/NAT/QoSの詳細、BGPネイバーパラメータ (タイマー・認証等)、FHRPのpriority値・preempt・トラッキング、STPの状態 (root/blocking は paths の注記で表す)、監視設定、ラック・パッチパネル収容、座標指定 |
...
- L2論理図 (トランクの通過VLAN表現)。lan-cable への segment 参照追加案があるが、L2図をスコープに含めるかの判断が先
- IPv6 / セカンダリIP。デュアルスタック案件を実際に書く需要が出た時点で図・表の表示設計と合わせて判断
- VRF の IF 単位所属 (現状は domains + logical link で代替)
...
| `link.via-forbidden` | error | via を logical/tunnel 以外で指定 |
| `ref.via` | error | via の参照先が clouds に存在しない |
| `link.port-reuse` | error | 同一物理ポートを複数の物理linkが使用 |
| `link.direction-forbidden` | error | direction: forward を logical 以外に指定 |
| `ref.redundancy-member` | error | 冗長グループのメンバー機器が存在しない |
| `redundancy.member-duplicate` | error | 同一グループ内で同じ機器を2回参照 |
| `redundancy.multi-stack` | error | 1機器が複数の stack グループに所属 (FHRPの複数所属は正当) |
| `redundancy.fhrp-only` | error | kind: stack に protocol / group / vip を指定 |
| `redundancy.cross-site` | warning | 冗長グループのメンバーが複数拠点にまたがる (枠表示は同一拠点のみ) |
| `redundancy.vip-segment` | warning | vip がメンバーIFのどのネットワーク (IF/セグメントのCIDR) にも含まれない |
| `ref.domain` | error | link の domain 参照先が domains に存在しない |
| `domain.name-required` | error | domain に name と protocol のどちらも無い |
| `domain.attr-mismatch` | error | area を ospf 以外 / asn を bgp 以外に指定 |
| `ref.redistribution-domain` / `ref.redistribution-device` | error | 再配布の from/to/devices の参照先が存在しない |
| `redistribution.same-domain` | error | 再配布の from と to が同一 |
| `redistribution.device-not-in-domain` | warning | 再配布機器が from/to 両ドメインの link 端点になっていない |
...
uv run nwdsl render   examples\stress\ring.yaml -o diagrams --format svg   # 内蔵SVGエンジン (D2不要, ADR-0008)
uv run nwdsl tables   examples\sample-corp\network.yaml -o diagrams\tables.md      # 全表 (最大8表)
...
| `tables.py` | 設計書向けの表 (最大8表) をMarkdown生成 (接続先・収容先・VIP所属はlinks/segmentsから逆引き) |
...
RedundancyGroup.members ──▶ Device (VIPの所属セグメントはCIDR包含で導出)
Redistribution ──▶ Domain (from/to) / Device (devices)
