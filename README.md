...
```powershell
uv sync                              # .venv作成 + 依存関係インストール (uv.lockから再現可能)
...
`uv run` は毎回ロックファイルと環境の同期を自動確認してから実行するため、PATHを通す作業も `Activate.ps1` も不要(PowerShellの実行ポリシーで弾かれる問題も起きない)。毎回 `uv run` と書くのを省きたい場合だけ `.\.venv\Scripts\Activate.ps1` で一度activateすれば、以降は `nwdsl ...` と直接呼べる。

[uv](https://docs.astral.sh/uv/) が無い場合は pip でも同様に動く: `python -m venv .venv` → `.\.venv\Scripts\pip install -e .` → `.\.venv\Scripts\Activate.ps1`(以降 `nwdsl ...` と読み替え。実行ポリシーで `Activate.ps1` 自体が弾かれる場合は `.\.venv\Scripts\nwdsl.exe ...` とフルパスで呼ぶ)。
