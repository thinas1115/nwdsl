...
| HSRP/VRRPペアやスタックが図で分からない | `redundancy_groups` で構造化。図では点線枠+Act/Sbyバッジ、VIPはIP設計表にも自動反映。スタックは物理図=2筐体・論理図=1筐体に描き分け([ADR-0010](docs/adr/0010-redundancy-groups.md)) |
| ルート再配布・デフォルトルートの矢印を論理図に書けない | `redistributions` でASBRに「再配布: OSPF ⇄ BGP」バッジ、`links[].direction` で有向の論理隣接([ADR-0011](docs/adr/0011-routing-domains.md) / [ADR-0012](docs/adr/0012-directed-logical.md)) |
...
OSPFエリア色分け・BGPピアリングの網経由描画・HSRP冗長・再配布:

```yaml
domains:
  - {id: area0, protocol: ospf, area: 0}       # 表示名 "OSPF Area 0" を自動生成
  - {id: as65000, protocol: bgp, asn: 65000}

links:
  - {type: logical, endpoints: ["hq-rt01", "dc-rt01"], domain: area0}
  - {type: logical, endpoints: ["dc-rt01", "aws-vgw01"], via: ipvpn, domain: as65000}

redundancy_groups:                             # 図では点線枠+Act/Sby、VIPは表にも載る
  - id: hq-wan
    protocol: hsrp
    vip: 10.1.0.1
    members:
...
redistributions:                               # ASBRノードに「再配布: ⇄」バッジ
  - {from: area0, to: as65000, devices: [dc-rt01], mutual: true}
```
...
| [complex-lan](examples/complex-lan/) | 2拠点19台 | 多段LAN (FW/コア冗長→ディストリ→アクセス、DMZ、VLAN別HSRP) |
| [stack-core](examples/stack-core/) | 1拠点5台 | スタックの描き分け (物理図=2筐体+枠 / 論理図=1筐体、クロススタックLAGの×N集約) |
...
- [実務パターン集](docs/patterns.md) — 冗長構成・スタック・網接続・専用線・DMZ・OSPFエリア・再配布等のパターン集
...
examples/           # 8サンプル(2〜50台規模) + stress/(描画エンジン限界検証)
